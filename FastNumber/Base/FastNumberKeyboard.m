...
typedef NS_ENUM(NSUInteger, FastNumberKeyboardButton) {
    FastNumberKeyboardButtonNumberMin,
    FastNumberKeyboardButtonNumberMax = FastNumberKeyboardButtonNumberMin + 10, // Ten digits.
    FastNumberKeyboardButtonBackspace,
    FastNumberKeyboardButtonDone,
    FastNumberKeyboardButtonSpecial,
    FastNumberKeyboardButtonDecimalPoint,
    FastNumberKeyboardButtonNone = NSNotFound,
};

@interface FastNumberKeyboard () <UIInputViewAudioFeedback, UITextInputDelegate>
...
@implementation FastNumberKeyboard

static const NSInteger FastNumberKeyboardRows = 4;
static const CGFloat FastNumberKeyboardRowHeight = 55.0f;
static const CGFloat FastNumberKeyboardPadBorder = 7.0f;
static const CGFloat FastNumberKeyboardPadSpacing = 8.0f;
...
- (void)_commonInit
{
    ...
    [self setReturnKeyButtonStyle:FastNumberKeyboardButtonStyleDone];
...
- (void)_configureButtonsForCurrentStyle
{
    ...
    const NSInteger numberMin = FastNumberKeyboardButtonNumberMin;
    const NSInteger numberMax = FastNumberKeyboardButtonNumberMax;
    ...
    for (FastNumberKeyboardButton key = numberMin; key < numberMax; key++) {
        UIButton *button = [MMKeyboardButton keyboardButtonWithStyle:FastNumberKeyboardButtonStyleWhite];
    ...
    UIButton *backspaceButton = [MMKeyboardButton keyboardButtonWithStyle:FastNumberKeyboardButtonStyleGray];
    ...
    [buttonDictionary setObject:backspaceButton forKey:@(FastNumberKeyboardButtonBackspace)];
    
    UIButton *specialButton = [MMKeyboardButton keyboardButtonWithStyle:FastNumberKeyboardButtonStyleGray];
    
    [buttonDictionary setObject:specialButton forKey:@(FastNumberKeyboardButtonSpecial)];
    
    UIButton *doneButton = [MMKeyboardButton keyboardButtonWithStyle:FastNumberKeyboardButtonStyleDone];
    [doneButton.titleLabel setFont:doneButtonFont];
    [doneButton setTitle:UIKitLocalizedString(@"Done") forState:UIControlStateNormal];
    
    [buttonDictionary setObject:doneButton forKey:@(FastNumberKeyboardButtonDone)];
    
    UIButton *decimalPointButton = [MMKeyboardButton keyboardButtonWithStyle:FastNumberKeyboardButtonStyleWhite];
    ...
    [buttonDictionary setObject:decimalPointButton forKey:@(FastNumberKeyboardButtonDecimalPoint)];
...
- (void)_configureButtonsForKeyInputState
{
    ...
    MMKeyboardButton *button = self.buttonDictionary[@(FastNumberKeyboardButtonDone)];
    if (button) {
...
{
    __block FastNumberKeyboardButton keyboardButton = FastNumberKeyboardButtonNone;
    
    [self.buttonDictionary enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        FastNumberKeyboardButton k = [key unsignedIntegerValue];
        if (button == obj) {
            keyboardButton = k;
            *stop = YES;
        }
    ...
    if (keyboardButton == FastNumberKeyboardButtonNone) {
        return;
    }
    
    // Get first responder.
    id <UIKeyInput> keyInput = self.keyInput;
    id <FastNumberKeyboardDelegate> delegate = self.delegate;
    ...
    // Handle backspace.
    else if (keyboardButton == FastNumberKeyboardButtonBackspace) {
    ...
    // Handle done.
    else if (keyboardButton == FastNumberKeyboardButtonDone) {
    ...
    // Handle special key.
    else if (keyboardButton == FastNumberKeyboardButtonSpecial) {
    ...
    // Handle .
    else if (keyboardButton == FastNumberKeyboardButtonDecimalPoint) {
...
- (void)setReturnKeyTitle:(NSString *)title
{
    ...
    if (![title isEqualToString:self.returnKeyTitle]) {
        UIButton *button = self.buttonDictionary[@(FastNumberKeyboardButtonDone)];
        if (button) {
...
- (void)setReturnKeyButtonStyle:(FastNumberKeyboardButtonStyle)style
{
...
- (void)setPreferredStyle:(FastNumberKeyboardStyle)style
{
...
NS_INLINE CGRect MMButtonRectMake(CGRect rect, CGRect contentRect, BOOL usesRoundedCorners){
    ...
    if (usesRoundedCorners) {
        CGFloat inset = FastNumberKeyboardPadSpacing / 2.0f;
...
- (void)layoutSubviews
{
    ...
    if ([UITraitCollection class]) {
        ...
        if (hasMargins || isIdiomPad) {
            ...
            const BOOL prefersPlainButtons = (self.preferredStyle == FastNumberKeyboardStylePlainButtons);
            const BOOL prefersRoundedButtons = (self.preferredStyle == FastNumberKeyboardStyleRoundedButtons);
    ...
    const CGFloat spacing = (usesRoundedButtons) ? FastNumberKeyboardPadBorder : 0.0f;
    ...
    const CGFloat rowHeight = CGRectGetHeight(contentRect) / FastNumberKeyboardRows;
    ...
    // Layout special key.
    UIButton *specialKey = buttonDictionary[@(FastNumberKeyboardButtonSpecial)];
    if (specialKey) {
        CGRect rect = (CGRect){ .size = numberSize };
        rect.origin.y = numberSize.height * 3;
    ...
    UIButton *decimalPointKey = buttonDictionary[@(FastNumberKeyboardButtonDecimalPoint)];
    if (decimalPointKey) {
        CGRect rect = (CGRect){ .size = numberSize };
        rect.origin.y = numberSize.height * 3;
    ...
    const int utilityButtonKeys[2] = { FastNumberKeyboardButtonBackspace, FastNumberKeyboardButtonDone };
    ...
    for (NSInteger idx = 0; idx < sizeof(utilityButtonKeys) / sizeof(int); idx++) {
        FastNumberKeyboardButton key = utilityButtonKeys[idx];
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    const CGFloat spacing = (interfaceIdiom == UIUserInterfaceIdiomPad) ? FastNumberKeyboardPadBorder : 0.0f;
    
    size.height = FastNumberKeyboardRowHeight * FastNumberKeyboardRows + (spacing * 2.0f);
